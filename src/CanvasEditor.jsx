class CanvasEditor {
  constructor(canvasRef, templateData) {
    ...
    this.initializeCanvas(); // Initialize the canvas on instantiation
  }

  // Initialize the canvas dimensions and draw the background and image mask
  initializeCanvas() {
  ...
  // Load and draw the image mask from the template data
  loadImageMask() {
    const img = new Image();
  ...
  // Draw the caption text on the canvas
  drawCaption() {
  ...
  // Draw the call-to-action (CTA) text on the canvas
  drawCTA() {
  ...
  // Break a long text into multiple lines based on a maximum number of characters per line
  ...
  // Calculate the width of the CTA box based on the text
  ...
  // Calculate the height of the CTA box based on the text
  ...
  // Update the caption text and redraw the canvas
  updateCaption(newCaption) {
    this.templateData.caption.text = newCaption;
    this.initializeCanvas();
  }

  // Update the CTA text and redraw the canvas
  updateCTA(newCTA) {
    this.templateData.cta.text = newCTA;
    this.initializeCanvas();
  }

  // Update the background color and redraw the canvas
